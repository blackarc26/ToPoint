...
export type CrawlParams = {
  ...
  /**
   * Extra constant columns appended to every scraped row (e.g. tagging which
   * research query / category a tweet came from). Keys become CSV columns.
   */
      ...
      while (
        ...
        if (response) {
          ...
          const rows = comingTweets.map((current: (typeof tweetContents)[0]) => {
            ...
            // Tag each row with research metadata (source query, category, ...)
            if (EXTRA_COLUMNS) {
              Object.assign(tweet, EXTRA_COLUMNS);
            }
